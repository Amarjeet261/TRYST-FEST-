...
export default function Gallery() {
  ...
  return (
    <section className="min-h-[730px] bg-[#1a0033] flex flex-col items-center pb-15 px-4 md:px-0">
      <h1 className="text-yellow-500 text-[45px] font-bold text-center pt-19">Gallery</h1>
      <hr className="w-[85px] border-t-4 border-yellow-500 mx-auto pb-8" />
      <p className="text-white text-center text-[22px]">Relive the memories from preview edition of TRYST through our Gallery</p>
      <div className="h-[40px] w-full max-w-[500px] border-2 text-white flex items-center justify-between px-3 mt-[25px] rounded-lg">
        {Datas.map((iteme, index) => (
          <button className="hover:bg-yellow-500 px-4 rounded-lg transition duration-300" onClick={() => { updateGallery(index + 1) }}>{iteme.year}</button>
        ))}
      </div>
     { setGallery == 1 ? ( <div className="flex flex-wrap gap-5 justify-center mt-5">
        {Concert.gallery1.map((picks) => {
          return(
            <img  src={picks.img} alt="Concert" className="h-[250px] w-[90%] sm:w-[400px] rounded-lg cursor-pointer hover:scale-105 transition duration-300" />
          )
        })}
        </div>):null}
        { setGallery == 2 ? ( <div className="flex flex-wrap gap-5 justify-center mt-5">
        ...
        { setGallery == 3 ? ( <div className="flex flex-wrap gap-5 justify-center mt-5">
